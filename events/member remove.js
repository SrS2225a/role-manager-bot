        ...
        try {
            ...
            if (invite.rowCount > 0) {
                ...
                if (new Date(Date.now() - member.joinedAt) < 120000) {
                    await db.query("UPDATE invite SET amount3 = amount3 + 1 WHERE guild = $1 and invite.invite = $2", [member.guild.id, invite.rows[0].invite])
                } else {
                    await db.query("UPDATE invite SET amount2 = amount2 + 1 WHERE guild = $1 and invite.invite = $2", [member.guild.id, invite.rows[0].invite])
                }
            }
            // end invite leaves
