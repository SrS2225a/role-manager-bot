                ...
                async with cursor.transaction():
                    ...
                    for user in await afk.fetch(message.guild.id):
                        ...
                        # checks if the mentioned user is afk, and if they are, tell the member that mentioned them
                            ...
                            await channel.send(
                                f"{message.author.mention} {him} is currently AFK with the reason: {discord.utils.escape_mentions(user[1])}!")
        ...
        if before.channel != after.channel:
            async with self.bot.db.acquire() as cursor:
                async with cursor.transaction():
                    ...
                    if dateval is not None and after.channel is None:
                    ...
                    elif dateval is None and after.channel is not None:
            ...
            async with cursor.transaction():
                ...
                if exists is not None:
                    ...
                    await cursor.execute(
                                ...
                                "VALUES($1, $2, $3, $4, $5, $6, $7)", member.guild.id, invite.inviter.id,
